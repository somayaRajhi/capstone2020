...
def test_client_id(get_docket):
    assert get_docket[0]['client_id'] == 1


def test_folder_name(get_docket):
    assert get_docket[0]['data'][0]['folder_name'] == 'test/123/'


def test_file_name(get_docket):
    assert get_docket[0]['data'][0]['file_name'] == 'basic_docket.json'
...
def test_file_contents(get_docket):
    assert get_docket[0]['data'][0]['data']['file_contents']['data'] == 'STUFF'
