...
def test_good_response():
    with requests_mock.Mocker() as mock:
        mock.get(URL + API_KEY + "&docketID=" + DOCKET_ID, json='The test is successful')
...
def test_bad_docket_id():
    with requests_mock.Mocker() as mock:
        ...
        mock.get(URL + API_KEY + "&docketID=" + bad_docket,
                 json='The test should yield a bad id', status_code=404)

        with pytest.raises(reggov_api_docket_error.BadDocketID):
            response = get_docket(API_KEY, bad_docket)
...
def test_bad_docket_id_pattern():
    with requests_mock.Mocker() as mock:
        ...
        mock.get(URL + API_KEY + "&docketID=" + bad_docket,
                 json='The test should yield a bad id pattern', status_code=400)
...
def test_bad_API_KEY():
    with requests_mock.Mocker() as mock:
        mock.get(URL + 'INVALID' + "&docketID=" + DOCKET_ID,
                 json='The test should yield a bad api key', status_code=403)

        with pytest.raises(reggov_api_docket_error.IncorrectApiKey):
            response = get_docket('INVALID', DOCKET_ID)
...
def test_no_API_KEY():
    with requests_mock.Mocker() as mock:
        ...
        with pytest.raises(reggov_api_docket_error.IncorrectApiKey):
            response = get_docket('', DOCKET_ID)


def test_maxed_API_KEY():
    with requests_mock.Mocker() as mock:
        mock.get(URL + API_KEY + "&docketID=" + DOCKET_ID,
                 json='The test should yield a overused api key', status_code=429)
